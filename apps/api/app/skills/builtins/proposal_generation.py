...
2. 禁止编造：报价、工期、预算等具体数字必须标注"需进一步确认"；屏幕参数（尺寸/类型/点距/安装环境/观看距离等）绝对禁止编造——上下文已提供的必须原样使用，未提供的必须在 missing_info 中列出并标注"需进一步确认"
...
**幕墙项目**: 使用原始10章结构（不额外增加章节）""" + GLOBAL_CAPABILITY_CONSTRAINT
...
class ProposalGenerationSkill(BaseSkill):
        ...
        if project_id and context.db:
            ...
            if result.success and not result.output.get("content"):
                ...
                chat_result = await self._execute_chat_mode(
                    ...
                    screen_info=input_data.get("screen_info"),
                )
    ...
    async def _execute_chat_mode(
        ...
        *,
        ...
        # Inject venue & screen params (agent path carries them via input_data;
        # workspace DB path injects via _default_prompt variables instead).
        if screen_info:
            user_prompt += (
                f"\n\n---\n\n场地与屏幕参数：\n{self._render_screen_params(screen_info)}\n"
            )
...
- 不要编造案例"""
    ...
    async def _execute_db_mode(
        ...
        # Venue & screen parameters — domain-critical inputs (CLAUDE.md §7.2/§4.3).
        # Provided params must be used directly; only absent ones stay "待确认".
        screen_params = self._render_screen_params(project.screen_info)
    ...
    @staticmethod
    def _render_screen_params(screen_info) -> str:
        """Render venue & screen parameters into prompt text.

        Empty/None → a marker telling the LLM these remain 待确认 (so it does
        NOT fabricate them). Populated → a structured list to use directly.
        """
        from app.skills.builtins._screen import missing_screen_labels, render_screen_block

        missing = missing_screen_labels(screen_info)
        tail = (
            "\n  （以下未提供，需进一步确认：" + "、".join(missing) + "）"
            if missing
            else ""
        )
        return render_screen_block(
            screen_info,
            intro="已提供（请直接使用，不要标注待确认）：",
            empty_marker="（暂未提供 — 屏幕类型、尺寸、点距、安装环境、观看距离均需进一步确认，禁止编造）",
            tail=tail,
        )

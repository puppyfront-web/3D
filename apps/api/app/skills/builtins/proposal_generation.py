...
2. 报价、工期等必须标注"需进一步确认"，禁止编造具体数字；屏幕参数（尺寸/类型/点距/安装环境/观看距离等）若已在上下文中提供，必须直接使用，仅未提供的才标注"需进一步确认"
...
- 不要编造案例"""
    ...
    def _render_screen_params(screen_info) -> str:
        ...
        if not screen_info or not isinstance(screen_info, dict):
            return "（暂未提供 — 屏幕类型、尺寸、点距、安装环境、观看距离均需进一步确认，禁止编造）"
        ...
        provided_keys = {labels.get(k, k) for k, v in screen_info.items() if v}
        all_keys = set(labels.values())
        missing_keys = all_keys - provided_keys
        tail = (
            "\n  （以下未提供，需进一步确认：" + "、".join(sorted(missing_keys)) + "）"
            if missing_keys
            else ""
        )
        return "已提供（请直接使用，不要标注待确认）：\n" + "\n".join(rows) + tail

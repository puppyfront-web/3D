...
AI 生成内容不能直接作为最终交付结果。所有 AI 生成内容都必须支持人工编辑、人工审核、版本保存、反馈评分和导出前检查。

标记为需要人工确认的内容类别由 SOP quality_review 步骤定义，管理员可通过后台配置调整。Agent 行为规范参见 [AGENTS.md](AGENTS.md)。

### 3.5 缺失信息必须提示

如果用户没有提供关键资料，不允许模型编造，必须标记「需要进一步确认」。

缺失信息的具体类别和处理规则由 SOP 步骤的 rules 定义，管理员可通过后台配置调整。Agent 缺失信息处理规范参见 [AGENTS.md](AGENTS.md)。
...
导出前必须执行审核检查，检查项由 SOP quality_review 步骤定义，管理员可配置。标记为 `require_human_review` 的内容必须经人工确认后才能导出。具体检查规则参见 [AGENTS.md](AGENTS.md) 和 SOP 工作流配置。
...
详细禁止事项和 Agent 行为约束参见 [AGENTS.md](AGENTS.md)。
